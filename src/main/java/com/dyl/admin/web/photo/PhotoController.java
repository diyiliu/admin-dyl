package com.dyl.admin.web.photo;
...
public class PhotoController {
    ...
    public Integer delete(@PathVariable Long pid, @PathVariable Long id) throws Exception {
        ...
        String thumbDir = fullDir + "thumb" + File.separator;
    ...
    private String doPhoto(List<String> names, List<MemPhoto> photos) throws Exception {
        ...
        for (Iterator<MemPhoto> iterator = photos.iterator(); iterator.hasNext(); ) {
            ...
            String fullDir =  environment.getProperty("upload.photo");
            org.springframework.core.io.Resource res = new UrlResource(fullDir + photo.getFull());
            if (names.contains(originalName) && res.exists()) {
                String thumbDir = fullDir + "thumb";
                ...
                photo.setThumb(tempFile.getName());
    ...
    public Map upload(MultipartFile file, HttpSession session) throws Exception {
        ...
        photo.setFull(tempFile.getName());
        photos.add(photo);
